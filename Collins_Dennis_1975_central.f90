...
CONTAINS
  SUBROUTINE SOR_PHI(PHI, OMEGA, B, C, RHOC, RHO, EPPS, ISOR_PHI, MAXSOR, NR, &
    ...
    ISOR_PHI = 0
    ! If parabolic wall formula is used, loop interior only to NR-1;
    ! otherwise use full stencil up to NR (accurate but less stable at high D)
    ...
    PHI_SOR: DO
      ...
      DO I = 2, NR_INNER
        ...
        END DO
      END DO
      ! Near-wall (I=NR): parabolic profile ψ∝(1-r)² gives PHI(NR)≈0.25*PHI(NR-1)
      ! Used during D-stepping for stability; full stencil used during convergence
      IF (USE_PARABOLIC_WALL) THEN
        DO J = 2, NA
...
PROGRAM MAIN
    ...
    REAL(KIND=dp) :: EPS_OUT_CASES(3, NCASES)  ! outer iteration convergence (may be looser)
...
!------------------- Initialization and setup ---------------------------
    ...
    AA_REG   = 1.0E-12_dp
    IF (NR >= 40) THEN
      MAXSOR = 50000          ! Grid (c): finer grid needs more SOR iterations
      STEP_ITERS = 40         ! Grid (c): more iterations per D-step
    ELSE
      MAXSOR = 2500           ! Grid (b)
      STEP_ITERS = 20         ! Grid (b)
    END IF
    MAXOUT = 40000
    NOR = 3
...
!------------------- Case configuration ---------------------------------
    ...
    ! Outer iteration convergence thresholds (post-averaging check).
    ! Same as EPS for D<=2000; loosened for D>=3500 where grid (b) is too coarse
    ! for strict convergence (C&D only report grid (c) results for these D).
    ! Inner SOR still uses tight EPS via EPPS = 0.05*EPS.
    ...
    IF (NR >= 40) THEN
      ! Grid (c): tight EPS_OUT — Anderson handles convergence speed
      EPS_OUT_CASES(:,9)  = EPS_CASES(:,9)
      EPS_OUT_CASES(:,10) = EPS_CASES(:,10)
    ELSE
      ! Grid (b): loosened — grid too coarse for strict convergence at high D
    ...
    IF (NR >= 40) THEN
      ! Grid (c): RHO_W=1.7 works for D<=2000
    ...
    ELSE
      ! Grid (b): RHO_W=1.5 for stability
    ...
    ! Fine D-stepping (Delta_D=20 for D<=2000, Delta_D=10 for D>2000) handles convergence
    XI_CASES(:,1)  = (/0.1_dp, 0.1_dp, 0.9_dp, 0.1_dp/)      ! D=10
    XI_CASES(:,2)  = (/0.1_dp, 0.1_dp, 0.9_dp, 0.1_dp/)      ! D=96
    XI_CASES(:,3)  = (/0.1_dp, 0.1_dp, 0.9_dp, 0.1_dp/)      ! D=100
    XI_CASES(:,4)  = (/0.5_dp, 0.1_dp, 0.5_dp, 0.5_dp/)      ! D=250
    XI_CASES(:,5)  = (/0.5_dp, 0.1_dp, 0.5_dp, 0.5_dp/)      ! D=500
    XI_CASES(:,6)  = (/0.5_dp, 0.1_dp, 0.5_dp, 0.5_dp/)      ! D=605.72
    XI_CASES(:,7)  = (/0.5_dp, 0.1_dp, 0.5_dp, 0.5_dp/)      ! D=1000
    XI_CASES(:,8)  = (/0.5_dp, 0.1_dp, 0.5_dp, 0.5_dp/)      ! D=2000
    XI_CASES(:,9)  = (/0.5_dp, 0.1_dp, 0.5_dp, 0.5_dp/)      ! D=3500
    XI_CASES(:,10) = (/0.5_dp, 0.1_dp, 0.5_dp, 0.5_dp/)      ! D=5000

    ! Fox correction smoothing parameter omega1 (C&D eq. 21)
    ! omega1 = 1.0 means full update (no smoothing)
    ! C&D: omega1=1.0 for D<=1000, 0.1 for D=2000, 0.05 for D=3500, 0.01 for D=5000
...
!------------------- D-stepping setup (C&D p.141) -----------------------
! For D > 1000, C&D increment D in steps from the previous converged value.
! Delta_D = 20 from D=1000 to D=2000, Delta_D = 10 for higher values.
! At each intermediate D, perform STEP_ITERS outer iterations.
! Full convergence only at the target D.

      IF (D_TARGET > 1000._dp .AND. ctr > 1) THEN
        STEPPING = .TRUE.
        D_CURRENT = DSTART   ! start from previous case's D
      ELSE
        STEPPING = .FALSE.
        D_CURRENT = D_TARGET
      END IF
      ...
      ! Fox correction initial values for this case
      ! C&D: For D>1000 with D-stepping, solve uncorrected first (C0=E0=0),
      ! then add corrections after reaching target D.
      ! For D<=1000, carry corrections from previous case as initial guess (T-0003).
      IF (STEPPING) THEN
        C0_CORR(:,:) = 0.0_dp
        E0_CORR(:,:) = 0.0_dp
      ELSE
...
!===== C&D iteration order: W -> Omega -> PHI (Section 4, p.140) =====
! Key: PHI(:,:,3) = old PHI here (PHI SOR runs last).
! This gives a one-iteration lag in the wall BC, which stabilizes high D.
...
!------------------- SOR for OMEGA --------------------------------------
        ...
        ! Uses PHI(:,:,3) which at this point is still OLD PHI (PHI SOR hasn't run yet)
        ! This matches C&D's iteration order W->Omega->PHI
        DO J = 2, NA
...
!------------------- SOR for PHI (last in C&D order) --------------------
! Uses NEW Omega (just computed above) as source for PHI equation.
        ...
        ! PHI source from NEW Omega (C&D: phi uses recently computed Omega)
        DO I = 2, NR
          DO J = 2, NA
...
!------------------- 2-cycle averaging for D>=2000 ----------------------
! Kill period-2 oscillation by averaging current and previous outer iterates.
! For eigenvalue lambda~=-1: x_avg = (x_n + T(x_n))/2 ~= x* (fixed point).
...
! Applied during BOTH D-stepping and convergence to stabilize full stencil.
        ...
        ! 2-cycle averaging threshold: finer grid resolves more instability modes
...
!------------------- D-stepping check -----------------------------------
        ...
        IF (STEPPING) THEN
          ...
          IF (MOD(step_count, STEP_ITERS) == 0) THEN
            IF (NR >= 40) THEN
              D_STEP = 5._dp      ! Grid (c): finer D-stepping
            ELSE
              D_STEP = 10._dp     ! Grid (b)
            END IF
            D_CURRENT = MIN(D_CURRENT + D_STEP, D_TARGET)
            D = D_CURRENT
            DDRDAM = D * DRDAM
            DDR2 = D * DR ** 2
            ! Print diagnostic at each D increment
...
! solution and check if another correction iteration is needed.
      ...
      ! Save uncorrected solution after first outer iteration converges.
      ! This is used as initial guess for the next case's D-stepping,
      ! which resets corrections to zero and needs a consistent starting point.
      IF (CORR_ITER == 0) THEN
        PHI_UNCORR(:,:) = PHI(:,:,3)
        W_UNCORR(:,:) = W(:,:,3)
...
! For D>1000 cases, save the UNCORRECTED solution as initial guess.
! D-stepping resets corrections to zero, so the initial guess must be
! consistent with zero corrections (not the corrected solution).
! For D<=1000, corrections carry over, so save the corrected solution.
